...
const DealsByYearBarChart = ({ selectedCompany }) => {
  ...
  useEffect(() => {
    if (data?.data?.data) {
      ...
      const maxCount = Math.max(...deals.map((lead) => lead.count), 10); // Default to 10 if no data
      const tickAmount = Math.ceil(maxCount / 5); // Aim for ~5 ticks

      // Extract months and counts from deals
      const months = [
        "Jan",
        "Feb",
        "Mar",
      ...
      const chartSeries = months.map((month, index) => {
        const monthData = deals.find((item) => item.month === index + 1);
...
export default DealsByYearBarChart;
